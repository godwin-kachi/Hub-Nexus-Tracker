...
class User
{
    ...
    function getUser()
    {
        ...
        if ($this->user_id != NULL) {
            ...
            $query = "SELECT user_id, admin_no, firstname, lastname, dob, image, class, guardian_name, guardian_phone, guardian_email, guardian_address, guardian_rel, active, created_at, updated_at  FROM " . $this->table_name . " WHERE admin_no=:admin_no";
            ...
            return ["output"=>"Please provide a valid user ID or Admission number","outputStatus"=>1400];
        ...
        try {
            ...
            if ($stmt->execute()) {
                ...
                if ($setId) {
                    ...
                    return ["output"=>$user_stmt['output'],  "outputStatus"=>1000];
                } elseif (!$setId) {
                    return ["output"=>false, "error"=>"user created but Admission Number generation failed. Please update manually", "outputStatus"=>1200];
                } else {
                    return $setId;
                }
            }
    ...
    public function changeActiveStatus($evcode)
    {
        ...
        // prepare query statement
        $atcive_stmt = $this->conn->prepare($query);

        // bind new values
        $atcive_stmt->bindParam(':admin_no', $this->admin_no);

        try {
            $atcive_stmt->execute();
            return ["output"=>$atcive_stmt, "outputStatus"=>1000];
    ...
    private function setLastuserAdminNo($lastId)
    {
        $offsetId = $lastId + 13;
        // update query
        $query = "UPDATE $this->table_name SET
